...
export default function Games() {
  ...
  if (gameMode === 'results') {
    ...
    useEffect(() => {
      try {
        const best = Number(localStorage.getItem("senti.games.bestAccuracy") || 0);
        if (percentage > best) localStorage.setItem("senti.games.bestAccuracy", String(percentage));
        localStorage.setItem("senti.games.lastAccuracy", String(percentage));
        localStorage.setItem("senti.games.lastTotal", String(answers.length));
      } catch {}
    }, []);
